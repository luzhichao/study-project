...
/**
 * 自定义KEY-VALUE,sheet写入处理器
 *
 * @author LZC
 * @version 1.1.2
 * @date 2022-11-08
 **/
public class CustomKVSheetWriteHandler implements SheetWriteHandler {
    /*** 动态下拉框的数据 **/
    private List<DropDownDO> list;

    public CustomKVSheetWriteHandler(List<DropDownDO> list) {
        this.list = list;
    }
    ...
    public void afterSheetCreate(WriteWorkbookHolder writeWorkbookHolder, WriteSheetHolder writeSheetHolder) {
        ...
        for (DropDownDO entry : list) {
            ...
            String refers = StrUtil.format(PoiConstant.DROP_DOWN_CELL_REFERS, sheetName, excelLine, excelLine, values.size());
